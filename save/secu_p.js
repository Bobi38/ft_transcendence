import {bcrypt, express, jwt, crypto,  nodemailer, validator, secret } from '../web/back/auth/src/routes/index_p.js';

import {User, PswEmail} from '../web/back/auth/src/routes/index_p.js';
...
router.post('/recovery/password', async (req, res) => {
    try{
        const {mail} = req.body;
        console.log("API /api/secu/recovery/password " + mail);
        if (!validator.isEmail(mail)){
            return res.status(400).json({ success: false, message: 'Invalid email format' });
        }
        const result = await User.findOne({ where: { mail: mail } });
        ...
        console.log("API /api/secu/recovery/password " + code);
        const CrypPass = await bcrypt.hash(code, 10);
        const check = await PswEmail.findOne({ where: { idUser: result.id, type: 2 } });
        console.log("API /api/secu/recovery/password " + check);
		console.log("crypt ", CrypPass);
		if (check){
			console.log("je suis dans destroy")
          	await check.destroy();
		}
        const re = await result.createCode({type: 2, Code : CrypPass, DateCreate: new Date()});
		console.log(re.Code);
        const token = jwt.sign({id: result.id}, secret, {expiresIn: '12h'});
