...
router.get('/get_stat', async (req, res) => {
    ...
    try{
          ...
          console.log("API/morp get_stat(1) called");


          let to_search = req.query.name;
          console.log("API/morp get_stat(2) to_search:", to_search);
...
router.get('/get_history/:page', async (req, res) => {
    try{
        console.log("API/morp get_history(1) called");

        const page = parseInt(req.params.page)
        if (page < 0)
          return (res.status(404).json({success: false, message: `unknow page_nbr=${page_nbr}`}))
        console.log("API/morp get_history(2.params) page:", page);

        const limit = parseInt(req.query.limit)
        console.log("API/morp get_history(2.query) limit:", limit);

        let to_search = req.query.name;
        console.log("API/morp get_history(2.query) to_search:", to_search);

        if (to_search !== undefined){

          // console.log("API/morp get_history(2.info.1)");
          to_search = await User.findOne({where: {name: to_search}})

          console.log("API/morp get_history(2.info.1) to_search:",to_search)
          if (to_search === null)
            return (res.status(404).json({success: false, message: `Hmmm il n'y a pas de user de ce nom..`}));
        
        } else {

          // console.log("API/morp get_history(2.info.2)");
          const token = req.cookies.token;
          const decoded = jwt.verify(token, secret);
          to_search = await User.findOne({where: {id: decoded.id}})
        ...
        console.log("API/morp get_history(3)");
    ...
    { model: User, as: 'loserUser', attributes: ['name'] },
        ...
        if (!result_history.length)
          return res.status(200).json({success: false, message: "API/morp get_history(catch) "});
        ...
        return res.status(500).json({success: false, message: "API/morp get_history(catch) " + err});
    }
});
