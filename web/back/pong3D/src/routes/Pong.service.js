    ...
    static async get_stat(to_search, token) {
        try{
            if (to_search !== undefined){
				// console.log("API/pong get_stat(2.info.1)");
				to_search = await User.findOne({where: {name: to_search}})
				// console.log("API/pong get_stat(2.info.1) to_search:",to_search)
				if (to_search === null)
					return ({success: false, message: `Hmmm no user with this name..`, code: 404});
            } else {
				// console.log("API/pong get_stat(2.info.2)");
				...
				// console.log("API/pong get_stat(2.info.2) to_search:",to_search)
				if (to_search === null)
					return ({success: false, message: `Hmmm no user with this name..`, code: 404});
            }
            console.log("API/pong get_stat(3)");
    ...
    static async get_history(to_search, page, limit, token) {
        try{
            if (to_search !== undefined){
                ...
                const decoded = user.user;
                to_search = await User.findOne({where: {id: decoded.id}})
                // console.log("API/pong get_history(2.info.2) to_search:",to_search)
                if (to_search === null)
                    return ({success: false, message: `no user with this name..`, code: 404});
            }
            ...
            console.log("API/pong get_history(4) result_history", result_history);
            ...
            return ({success: false, message: "API/pong get_history(catch) " + err, code: 500});
        }
    }
}
