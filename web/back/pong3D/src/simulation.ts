...
import { Environment, ToVec3 } from "./environment.js";
...
export class Simulation {
    ...
    constructor(room: MyRoom, state: MyRoomState, matchStats: Map<string, PlayerStats>) {
        ...
        this._ball = new Ball(new Vector3(state.ball.position.x, state.ball.position.y, state.ball.position.z),
          Vector3.Zero(), config.ballDiameter, scene);
        this._runSimulation();
    }
    ...
    private _executeStep() {
        ...
        const newPos = oldPos.add(this._ball.getVelocity().scale(config.timestep));
    ...
    if (winningSide) {
      ...
      if (winnerStats) {
        ...
        if (winningSide === "far") {
          ...
          if (this._state.score.teamFar >= config.pointsToWin) {
          ...
          if (this._state.score.teamNear >= config.pointsToWin) {
            winnerStats.hasWon = true;
            this._state.roomStatus = RoomStatus.WON;
            this._timeEnd = Date.now();
          }
...
private _resetBallAfterGoal(ballPos: Vector3) {
    ...
    if (!this._served) {
        ballPos = ToVec3(config.ballNearStart);
        this._state.ball.position.z = ballPos.z;
        this._served = true;
    } else {
        ballPos = ToVec3(config.ballFarStart);
        this._state.ball.position.z = ballPos.z;
        this._served = false;
    }
    this._ball.setPhysicsBodyPosition(ballPos);
