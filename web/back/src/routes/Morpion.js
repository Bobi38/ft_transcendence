...
router.get('/get_stat', async (req, res) => {
    ...
    try{
          ...
          console.log("API get_stat(1) called");


          let to_search = req.query.name;
          console.log("API get_stat(2) to_search:", to_search);
          if (to_search !== undefined){

            // console.log("API get_stat(2.info.1)");
            to_search = await User.findOne({where: {name: to_search}})

            console.log("API get_stat(2.info.1) to_search:",to_search)
            if (to_search === null)
                return (res.status(404).json({success: false, message: `Hmmm il n'y a pas de user de ce nom..`}));
        
        } else {

            // console.log("API get_stat(2.info.2)");
            const token = req.cookies.token;
            const decoded = jwt.verify(token, secret);
            to_search = await User.findOne({where: {id: decoded.id}})

            console.log("API get_stat(2.info.2) to_search:",to_search)
            if (to_search === null)
                return (res.status(404).json({success: false, message: `Hmmm il n'y a pas de user de ce nom..`}));

        }

        console.log("API get_stat(3)");
...
router.get('/get_history/:page', async (req, res) => {
    try{
        ...
        if (to_search !== undefined){
          ...
          // console.log("API get_history(2.info.1)");
          to_search = await User.findOne({where: {name: to_search}})

          console.log("API get_history(2.info.1) to_search:",to_search)
          if (to_search === null)
            return (res.status(404).json({success: false, message: `Hmmm il n'y a pas de user de ce nom..`}));
        
        } else {

          // console.log("API get_history(2.info.2)");
          const token = req.cookies.token;
          const decoded = jwt.verify(token, secret);
          to_search = await User.findOne({where: {id: decoded.id}})

          console.log("API get_history(2.info.2) to_search:",to_search)
          if (to_search === null)
            return (res.status(404).json({success: false, message: `Hmmm il n'y a pas de user de ce nom..`}));
        }
    ...
    { model: User, as: 'loserUser', attributes: ['name'] },
        ...
        console.log("API get_history(4) result_history", result_history);

        if (!result_history.length)
          return res.status(200).json({success: false, message: "API get_history(catch) "});
        ...
        return res.status(500).json({success: false, message: "API get_history(catch) " + err});
    }
});
